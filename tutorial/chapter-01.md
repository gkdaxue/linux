...
2. 选择操作系统时, 一定要选择 **`稍后安装操作系统`** 
   ...
   >     如果在此步骤选择了下载的镜像文件, 有可能会被部署为 `精简模式的操作系统`, 很多软件都不会被安装, 这对于我们新手来说, 会导致在平时学习中遇到很多的问题.
...
## tty查看终端
查看当前在什么终端下面, 可以输入 tty 命令
```bash
[root@localhost ~]# tty
/dev/pts/0

## 从命令输出结果, 我们可以看出, 我们是 图形终端/模拟终端
```
...
# 交互式接口
  ...
     **因为终端没有办法进行交互**, 所以 `启动终端后, 在终端设备附加一个交互式应用程序`, 这样我们就可以通过终端, 来执行我们的操作. 
...
### tty命令
**tty命令显示终端机连接标准输入设备的文件名称.**
终端是用户与主机交互必然会用到的设备, 那么我们应该如何查看我们现在在什么终端下呢?
> tty
